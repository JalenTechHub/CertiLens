/**
 * CertiLens - Background Service Worker
 * 
...
async function runFullAnalysis(tabUrl, domData = {}) {
  ...
  // Check brand spoofing using ONLY page identity signals:
  // title, h1 headings, logo alt text, and meta description.
  // We intentionally do NOT use full body text because legitimate sites
  // (e.g. google.com search results) naturally reference other brands in content.
  let brandSpoofing = [];
  const identityText = [
    domData.title || '',
    ...(domData.headings || []),
    ...(domData.logoAlts || []),
    domData.metaDescription || '',
  ].join(' ').toLowerCase().trim();

  if (identityText.length > 0) {
    for (const b of BRAND_PATTERNS) {
      // Must be on the wrong domain to be considered spoofing
      const onOfficialDomain = b.domains.some(d =>
        hostname === d || hostname.endsWith('.' + d)
      );
      if (onOfficialDomain) continue;

      // Brand keyword must appear in the page's own identity signals
      const identityMatch = b.keywords.some(k => identityText.includes(k));
      if (!identityMatch) continue;

      // If the brand name appears in the domain itself, it's likely a legit
      // sub-service (maps.google.com, accounts.google.com, etc.)
      const domainHasBrandName = b.keywords.some(k => domain.includes(k));
      if (domainHasBrandName) continue;
...
chrome.runtime.onInstalled.addListener(async ({ reason }) => {
  if (reason === 'install') {
    ...
    console.log('[CertiLens] Installed successfully.');
