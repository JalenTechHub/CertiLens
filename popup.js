  ...
  $main.innerHTML = `
    <div class="empty-state">
      ...
      <div class="empty-desc">
        CertiLens can only analyze HTTP and HTTPS web pages.
...
function exportReport() {
  ...
  const hostname = (r.hostname || 'unknown').replace(/[^a-z0-9.-]/gi, '_');
  const ts = new Date().toISOString().slice(0, 10);
  const filename = `certilens-report-${hostname}-${ts}.txt`;

  // Primary: use chrome.downloads API (requires "downloads" permission in manifest)
  if (chrome.downloads && chrome.downloads.download) {
    chrome.downloads.download({ url: blobUrl, filename, saveAs: false })
      .catch(() => fallbackDownload(blobUrl, filename));
  } else {
    fallbackDownload(blobUrl, filename);
  }
}

/** Anchor-click fallback download — works even without downloads permission */
